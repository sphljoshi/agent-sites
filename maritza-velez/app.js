...
const SERVICE_ICONS = {
  ...
  personal: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="m3 10 9-7 9 7v10a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1z"/><path d="M12 11.5c.9-.9 1.7-1.3 2.5-1.3a2 2 0 0 1 2 2c0 1.5-1.6 2.9-4.5 4.8-2.9-1.9-4.5-3.3-4.5-4.8a2 2 0 0 1 2-2c.8 0 1.6.4 2.5 1.3Z"/></svg>',
...
const P = {
  ...
  shield: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',
  key: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><circle cx="7.5" cy="15.5" r="4.5"/><path d="m10.7 12.3 8.3-8.3"/><path d="m16.5 6.5 2.5 2.5"/><path d="m14 9 2.5 2.5"/></svg>',
  water: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2.5s6 6.6 6 10.5a6 6 0 0 1-12 0c0-3.9 6-10.5 6-10.5z"/><path d="M2 20.5c1.8 0 1.8 1.5 3.7 1.5s1.8-1.5 3.6-1.5 1.8 1.5 3.7 1.5 1.8-1.5 3.6-1.5 1.8 1.5 3.7 1.5"/></svg>',
  paw: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="6.5" cy="9.5" rx="1.8" ry="2.4"/><ellipse cx="10.5" cy="6.5" rx="1.8" ry="2.4"/><ellipse cx="15" cy="6.5" rx="1.8" ry="2.4"/><ellipse cx="18.5" cy="10" rx="1.8" ry="2.4"/><path d="M12.5 13c2.6 0 5 2 5 4.3 0 1.8-1.4 2.7-2.8 2.7-1 0-1.4-.6-2.2-.6s-1.2.6-2.2.6c-1.4 0-2.8-.9-2.8-2.7C7.5 15 9.9 13 12.5 13z"/></svg>',
  rv: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M2 16V8a2 2 0 0 1 2-2h11l5 5v5h-2"/><circle cx="8" cy="16" r="2"/><circle cx="17" cy="16" r="2"/><path d="M10 16h5"/><rect x="5" y="9" width="4" height="3"/></svg>',
  gavel: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="m14 12-8.5 8.5a2.1 2.1 0 0 1-3-3L11 9"/><path d="m12.5 4.5 7 7"/><path d="m15.5 1.5 7 7"/><path d="m10.5 6.5 7 7"/></svg>',
  badge: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="6"/><path d="m8.5 14.5-1.5 7 5-2.5 5 2.5-1.5-7"/></svg>',
  hammer: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M14.5 4.5 17 2l5 5-2.5 2.5z"/><path d="m17 7-3.5 3.5"/><path d="M13.5 10.5 4 20a2.1 2.1 0 0 0 3 3l9.5-9.5z"/><path d="M10.5 7.5 13 5l4 4-2.5 2.5z"/></svg>',
...
const POLICY_ICONS = {
  ...
  annuities: P.coins,
  "long-term disability": P.wheelchair,
  "short-term disability": P.bandage,
...
/* =============================================================
   ...
   ============================================================= */
const PAGE_TITLES = {
  index: `${AGENT.name} | ${AGENT.specialty} in ${AGENT.location}`,
  about: `About ${AGENT.name} | ${AGENT.agency || AGENT.name} in ${AGENT.location}`,
...
function buildFooter() {
  ...
  footer.innerHTML = `
    ...
    <div class="wrap footer__bottom">
      <span>© ${year} ${AGENT.copyright || AGENT.name}. All rights reserved.</span>
    </div>`;
}
...
function initHome() {
  ...
  if (hero && AGENT.heroImage) {
    hero.style.backgroundImage = `linear-gradient(90deg, rgba(0,0,0,.82) 0%, rgba(0,0,0,.35) 60%, rgba(0,0,0,.15) 100%), url('${AGENT.heroImage}')`;
  }
  const ht = document.getElementById("hero-title");
  if (ht)
    ht.innerHTML = AGENT.heroTitle || `Insurance built around you.`;
  ...
  if (grid) {
    ...
    grid.innerHTML = keys
      .map((k, i) => {
        ...
        return `<a class="coverage__card coverage__card--${v}" href="services.html">
  ...
  container.innerHTML = serviceKeys()
    .map((k) => {
      ...
      return `<section class="service-block">
        <span class="service-block__eyebrow">${s.eyebrow || serviceLabelShort(k)}</span>
        <h2 class="service-block__title">${s.label}</h2>
        <p class="service-block__lead">${s.description}</p>
        <div class="service-block__body">
          <div class="service-block__media">
            <img src="${img}" alt="${s.label} — ${AGENT.name}, ${AGENT.location}">
            <div class="service-block__overlay">
...
function initSectionAlternation() {
  const sections = Array.from(
    document.querySelectorAll("section.section, section.stats-band")
  ).filter((s) => getComputedStyle(s).display !== "none");

  // First content section after the hero is muted, then alternate.
  sections.forEach((s, i) => {
    s.classList.toggle("section--soft", i % 2 === 0);
  });
}
