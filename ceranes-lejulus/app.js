...
const ICONS = {
  ...
  /* Detailed variants for the stat cards */
  clockDetail: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 4.5V6"/><path d="M12 18v1.5"/><path d="M4.5 12H6"/><path d="M18 12h1.5"/><path d="M12 8v4l3 2"/><circle cx="12" cy="12" r="0.75" fill="currentColor" stroke="none"/></svg>',
  locationDetail: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6.4-8 12.5-8 12.5S4 16.4 4 10a8 8 0 0 1 16 0z"/><circle cx="12" cy="10" r="3.25"/><circle cx="12" cy="10" r="0.75" fill="currentColor" stroke="none"/></svg>',
  verifiedBadge: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z"/><path d="m9 12 2 2 4-4"/></svg>',
  shieldDetail: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M8.75 11.75l2.25 2.25 4.25-4.5"/></svg>',
};
...
const POLICY_ICONS = {
  ...
  "bop property": P.building,
  "worker's compensation": P.workers,
  "errors and omissions": P.liability,
  "accountants professional": P.chart,
  "agricultural output program": P.farm,
  "agriculture liability": P.farm,
  "agriculture package": P.farm,
  "agriculture property": P.farm,
  "agriculture scheduled & unscheduled personal property": P.farm,
  "architects professional": P.building,
  artisans: P.briefcase,
  "boiler and machinery": P.fire,
  ...
  "employment practices liability": P.users,
  "equine liability": P.farm,
  "excess liability": P.umbrella,
  fiduciary: P.chart,
  "garage and dealers": P.car,
  "lawyers professional": P.liability,
  "livestock mortality": P.farm,
  "media professional": P.briefcase,
  "medical professional liability": P.heart,
  "miscellaneous professional liability": P.briefcase,
  "motor carrier": P.car,
  "motor truck cargo": P.package,
  "ocean marine": P.boat,
  "personal inland marine": P.package,
  "physicians and surgeons": P.heart,
  "railroad protective liability": P.liability,
  "small farm/ranch": P.farm,
  "special event": P.users,
  "special multi-peril": P.umbrella,
  surety: P.liability,
  truckers: P.car,
  "wind policies": P.umbrella,
  "workplace violence": P.workers,
  // life
  life: P.heart,
  "term life": P.clockP,
  "whole life": P.heart,
...
function brandMarkup(inverse) {
  const logoSrc = inverse ? AGENT.logoInverse || AGENT.logo : AGENT.logo;
  if (logoSrc) {
    return `<img src="${logoSrc}" alt="${AGENT.name}" class="brand-logo">`;
  }
  // Text brand, Policygenius-style: bold part + light part from config
  // (e.g. { bold: "Armine Insurance", light: "Agency" }); falls back to name.
  const b = AGENT.brand || { bold: AGENT.name, light: "" };
  return `<span class="brand-bold">${b.bold}</span><span class="brand-light">${b.light || ""}</span>`;
}
...
/* =============================================================
   NAV
   ============================================================= */
const SPECIALTY = AGENT.specialty || "Insurance";
const PAGE_TITLES = {
  index: `${AGENT.name} | ${SPECIALTY} in ${AGENT.location}`,
  about: `About ${AGENT.name} | ${SPECIALTY} in ${AGENT.location}`,
...
function buildFooter() {
  ...
  const navLinks = [
    ...
    .map(([href, label]) => `<a class="footer__nav-link" href="${href}">${label}</a>`)
    .join("");
  ...
  const contactDetails = `
    <div class="footer__contact">
      <a class="footer__contact-item" href="${AGENT.phoneHref}">${ICONS.phone}<span>${AGENT.phone}</span></a>
      <a class="footer__contact-item" href="mailto:${AGENT.email}">${ICONS.email}<span>${AGENT.email}</span></a>
    </div>`;

  const addressRow = `
    <span class="footer__contact-item footer__address">${ICONS.location}<span>${AGENT.address || AGENT.location}</span></span>`;
  ...
  footer.innerHTML = `
    <div class="wrap">
      <div class="newsletter">
        <div class="newsletter__text">
          <h2 class="newsletter__title">Getting started is easy</h2>
          <p class="newsletter__sub">Whether you still have questions or have an idea of what you need, we'll match you with a professional in your local area.</p>
          <a class="btn-primary newsletter__cta" href="services.html">Get a Quote ${ICONS.arrow}</a>
        </div>
        <div class="newsletter__media"><img src="images/footer-cta-image.webp" alt="" aria-hidden="true" /></div>
      </div>
    </div>
    <div class="wrap footer__nav-row">
      <div class="footer__brand-col">
        <a href="index.html" class="footer__brand">${brandMarkup(true)}</a>
        ${contactDetails}
      </div>
      <div class="footer__nav-col">
        <nav class="footer__nav">${navLinks}</nav>
        ${addressRow}
      </div>
    </div>
    ...
    <div class="wrap footer__bottom">
      ${socialRow}
      <span>Copyright © ${year} ${AGENT.copyright || AGENT.name}. All Rights Reserved.</span>
    </div>`;
}
...
/* =============================================================
   ...
   ============================================================= */
function initHome() {
  /* Hero (v4): centered title over a wide rounded media card.
     heroTitle (HTML, with a <span class="emphasis"> word) comes from config;
     when absent, the baked HTML default stays. */
  const ht = document.getElementById("hero-title");
  if (ht && AGENT.heroTitle) ht.innerHTML = AGENT.heroTitle;
  const hs = document.getElementById("hero-subtitle");
  if (hs) hs.textContent = AGENT.tagline;

  /* Hero media slider — autoplays through the coverage images with a
     directional clip-reveal animation; the arrows steer it manually
     (which restarts the autoplay timer) and hovering pauses it.
     Starts on whichever image is baked into the HTML so first paint
     and hydration agree. Reduced motion: no autoplay, no reveal. */
  const media = document.getElementById("hero-media");
  if (media) {
    const img = media.querySelector("img");
    const slides =
      AGENT.heroSlides && AGENT.heroSlides.length
        ? AGENT.heroSlides.map((s) => ({
            src: s.src,
            alt: s.alt || `${AGENT.name}, ${AGENT.location}`,
          }))
        : serviceKeys().map((k) => ({
            src: AGENT.services[k].image,
            alt: `${AGENT.services[k].label}, ${AGENT.name}, ${AGENT.location}`,
          }));
    slides.forEach((s) => {
      new Image().src = s.src; // preload so reveals never flash empty
    });
    const reduced = window.matchMedia(
      "(prefers-reduced-motion: reduce)"
    ).matches;
    let idx = img
      ? slides.findIndex((s) => (img.getAttribute("src") || "") === s.src)
      : 0;
    if (idx < 0) idx = 0;
    let timer = null;
    const show = (n, dir) => {
      ...
      if (!reduced) {
        img.classList.remove(
          "hero__media-img--reveal-left",
          "hero__media-img--reveal-right"
        );
        void img.offsetWidth; // restart the CSS reveal animation
        img.classList.add(
          dir < 0
            ? "hero__media-img--reveal-left"
            : "hero__media-img--reveal-right"
    ...
    const play = () => {
      ...
      if (!reduced && slides.length > 1)
        timer = setInterval(() => show(idx + 1, 1), 5000);
    };
    const prev = media.querySelector(".hero__media-arrow--prev");
    const next = media.querySelector(".hero__media-arrow--next");
    if (prev)
    ...
    media.addEventListener("mouseleave", play);
    play();
  }

  /* Featured plan cards — one colored card per coverage category */
  const plans = document.getElementById("plans-cards");
  if (plans) {
    const variants = ["dark", "gold", "soft"];
    plans.innerHTML = serviceKeys()
      .map((k, i) => {
        ...
        return `<a class="plan-card plan-card--${v}" href="${link}"${target}>
          ...
          <p class="plan-card__desc">${s.cardDescription || s.description}</p>
          <span class="plan-card__meta"><strong>${s.items.length}</strong> policy options <span class="icon-circle">${ICONS.arrow}</span></span>
        </a>`;
      })
      .join("");
  }

  /* Experience highlight card (feature strip) */
  const xp = document.getElementById("xp-value");
  if (xp && AGENT.experience) xp.textContent = `${AGENT.experience}+`;

  /* Stat chips */
  const chips = document.getElementById("stat-chips");
  if (chips) {
    const icons = [ICONS.clockDetail, ICONS.verifiedBadge, ICONS.shieldDetail];
    chips.innerHTML = AGENT.stats
      .map(
        (s, i) =>
          `<div class="stat-chip"><span class="stat-chip__icon">${icons[i % icons.length]}</span><span class="stat-chip__value">${s.value}</span><span class="stat-chip__label">${s.label}</span></div>`
      )
      .join("");
  }
...
/* =============================================================
   ...
   V4: a centered testimonial "stage" showing one review at a time
   (initials avatar, quote, name, role) with dot pagination when
   there is more than one review.
   ============================================================= */
function initReviews() {
  ...
  const paint = () => {
    ...
    const dots =
      reviews.length > 1
        ? `<div class="testi__dots">${reviews
            .map(
              (_, i) =>
                `<button class="testi__dot${
                  i === idx ? " testi__dot--active" : ""
                }" aria-label="Review ${i + 1}"></button>`
            )
            .join("")}</div>`
        : "";
    grid.innerHTML = `<div class="testi__item">
        <span class="testi__avatar">${initials(r.name)}</span>
        <blockquote class="testi__quote">"${r.quote}"</blockquote>
        <span class="testi__name">${r.name}</span>
        ${r.role ? `<span class="testi__role">${r.role}</span>` : ""}
      </div>${dots}`;
    grid.querySelectorAll(".testi__dot").forEach((d, i) =>
...
function initAccordions() {
  ...
  items.forEach((item) => {
    ...
    btn.addEventListener("click", () => {
      ...
      // Single-open per accordion group: only collapse siblings in the
      // same container (the page can hold several accordions, e.g. the
      // home "why choose" list and the FAQ list).
      items.forEach((other) => {
        if (other !== item && other.parentElement === item.parentElement) {
...
function initSectionAlternation() {
  // No alternation — sections keep the background baked into their HTML
  // (default white unless a section opts into .section--soft). The CTA
  // card always sits on white.
  document
    .querySelectorAll("section.bottom-cta")
    .forEach((s) => s.classList.add("bottom-cta--default"));
}
